...
(function($){
	if($('.navmenu_close').length < 1){
		...
		$('#header.renew171127').prepend('<a href="javascript:void(0)" class="navmenu_close close_x_btn" style="left:0;z-index:102;"></a>');
	}
	...
	var menunavWidth = $('.nav_menu').width();
	var menuslideMove = windowWidth - menunavWidth
	...
	$('.nav_main .btn_slide').bind('click',function(){
		if($(this).hasClass('open')){
			...
			// btn_slide renew171127
			if($(this).hasClass('renew171127')){

				setTimeout(function(){
					$('#header .nav_menu').addClass('slide-open').css({
						transform : 'translate3d('+menuslideMove+'px ,0 ,0)'
					});
				},100);
